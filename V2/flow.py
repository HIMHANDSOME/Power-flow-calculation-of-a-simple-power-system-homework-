...
# ===== 用户选择计算方法和存储方式 =====
print("\n请选择潮流计算方法：")
print("  1. Newton-Raphson法 (NR)")
print("  2. 快速解耦法 (Fast Decoupled, FDXB，标准的快速解耦法)")
print("  3. 快速解耦法 (Fast Decoupled, FDBX，电阻较大的网络中收敛性能更好)")
print("  4. 高斯-塞德尔法 (GS)")
choice = input("请输入选项编号 (1-4，默认1): ").strip()
if choice == '' or choice == '1':
    algorithm = 'nr'
    print("选择: Newton-Raphson法")
elif choice == '2':
    algorithm = 'fdxb'
    print("选择: 快速解耦法 (FDXB)")
elif choice == '3':
    ...
    algorithm = 'nr'
    print(f"无效输入 '{choice}'，使用默认Newton-Raphson法")

print("\n请选择矩阵存储方式：")
print("  1. 稀疏矩阵存储 (更高效)")
print("  2. 稠密矩阵存储 (默认)")
store_sparse_input = input("请输入选项编号 (1-2，默认2): ").strip()
store_sparse = (store_sparse_input == '1')
if store_sparse:
    print("选择: 稀疏矩阵存储")
else:
    print("选择: 稠密矩阵存储")
...
if store_sparse:
    Ybus = Ybus_sparse  # 保持稀疏矩阵
    print("\n=== 节点导纳矩阵 Ybus (稀疏矩阵，标幺值) ===")
    print("稀疏矩阵格式:", Ybus.format, "形状:", Ybus.shape)
    # 打印前几个非零元素
    print("非零元素数量:", Ybus.nnz)
    # 转换为稠密矩阵用于显示（仅显示部分）
...
def save_results_to_json(net, Ybus, Zb, algorithm='nr', store_sparse=False):
    """保存潮流计算结果到JSON文件"""
    ...
    # 处理Ybus矩阵，将复数转换为可序列化格式，支持稀疏矩阵
    import scipy.sparse as sp

    def serialize_ybus(ybus):
        """序列化Ybus矩阵，支持稀疏和稠密格式"""
        if sp.issparse(ybus):
            # 稀疏矩阵：存储为COO格式（行、列、实部、虚部）
            ybus_coo = ybus.tocoo()
            return {
                "format": "coo",
                "shape": ybus_coo.shape,
                "row": ybus_coo.row.tolist(),
                "col": ybus_coo.col.tolist(),
                "data_real": np.real(ybus_coo.data).round(6).tolist(),
                "data_imag": np.imag(ybus_coo.data).round(6).tolist()
            }
        else:
            # 稠密矩阵：存储为二维列表
            ...
            return {
                "format": "dense",
                "data": ybus_serializable
            }

    ybus_serialized = serialize_ybus(Ybus)
    ...
    results = {
        ...
        "load_results": [],
        "generator_results": [],
        "ybus_matrix": ybus_serialized,  # 使用处理后的矩阵（支持稀疏/稠密）
        "per_unit_parameters": {
...
# 调用保存函数
json_file = save_results_to_json(net, Ybus, Zb, algorithm, store_sparse)
