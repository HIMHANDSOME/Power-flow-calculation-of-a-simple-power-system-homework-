...
def parse_ybus_data(ybus_dict):
    """
    解析Ybus矩阵数据，支持多种格式：
    1. 旧格式：二维列表，每个元素为{"real": x, "imag": y}
    2. 新格式：包含"format"字段，可能是"dense"或"coo"
    """
    # 检查是否为旧格式（直接是列表）
    if isinstance(ybus_dict, list):
        return ybus_dict  # 旧格式，直接返回

    # 新格式：检查format字段
    if not isinstance(ybus_dict, dict):
        raise ValueError("Ybus数据格式无效")

    fmt = ybus_dict.get("format")
    if fmt == "dense":
        # 稠密格式：直接返回data字段
        return ybus_dict.get("data", [])
    elif fmt == "coo":
        # 稀疏COO格式：重建稠密矩阵
        import numpy as np
        shape = ybus_dict.get("shape", (0, 0))
        rows = ybus_dict.get("row", [])
        cols = ybus_dict.get("col", [])
        data_real = ybus_dict.get("data_real", [])
        data_imag = ybus_dict.get("data_imag", [])

        nrows, ncols = shape
        # 创建零矩阵（复数）
        dense = np.zeros(shape, dtype=complex)
        for r, c, real, imag in zip(rows, cols, data_real, data_imag):
            dense[r, c] = complex(real, imag)
        ...
        for i in range(nrows):
            ...
            for j in range(ncols):
                z = dense[i, j]
                row.append({"real": float(z.real), "imag": float(z.imag)})
            result.append(row)
        return result
    else:
        raise ValueError(f"未知的Ybus格式: {fmt}")
...
def main():
    ...
    ybus_raw = data["ybus_matrix"]

    # 解析Ybus矩阵（支持多种格式）
    try:
        ybus_data = parse_ybus_data(ybus_raw)
    except Exception as e:
        print(f"解析Ybus矩阵失败: {e}")
        return 1
